...
@Suite struct SessionStoreTests {
    ...
    @Test func preFeatureV6CacheDefaultsOnlyNewTouchedFileFields() throws {
        let validV6 = """
        {"version":6,"entries":{},"indexedIDs":[],"dirtyIDs":[],"pendingGhostDeletions":[],"codexTitles":{},"iconSources":{},"searchLaterPromptsEnabled":false,"laterPromptPendingPaths":[]}
        """
        let decoded = try JSONDecoder().decode(ScanCache.self, from: Data(validV6.utf8))
        #expect(decoded.touchedFileExtractionGeneration == 0)
        #expect(decoded.touchedFilePendingPaths.isEmpty)

        let corrupt = "{\"version\":6,\"entries\":{}}"
        #expect(throws: DecodingError.self) {
            try JSONDecoder().decode(ScanCache.self, from: Data(corrupt.utf8))
        }
    }
