...
@Suite struct CodexScannerTests {
    ...
    @Test func assistantReplyOptInAllowsOnlyVisibleMessageText() throws {
        ...
        // Message-shaped payload under a non-response envelope must fail closed.
        let foreignEnvelope = try Self.assistantMessage(["SECRET_FOREIGN_ENVELOPE"])
            .replacingOccurrences(of: "\"type\":\"response_item\"", with: "\"type\":\"future_item\"")
        let home = try makeHome(sessionLines: [
            Self.meta(),
            Self.userMessage("first prompt"),
