...
private struct AdvancedSettingsTab: View {
    ...
    var body: some View {
        ...
        .formStyle(.grouped)
        .task { await reloadSavedProjects() }
    }
    ...
    private func chooseSavedProject() {
        ...
        Task {
            do {
                try await AppCoordinator.shared.addSavedProject(path: url.path)
    ...
    private func removeSavedProject(_ path: String) {
        Task {
            do {
                try await AppCoordinator.shared.removeSavedProject(path: path)
                await reloadSavedProjects()
            } catch {
                savedProjectMessage = error.localizedDescription
            }
    ...
    private func moveSavedProject(from source: Int, to destination: Int) {
        Task {
            do {
                try await AppCoordinator.shared.moveSavedProject(from: source, to: destination)
                await reloadSavedProjects()
            } catch {
                savedProjectMessage = error.localizedDescription
            }
    ...
    private func reloadSavedProjects() async {
        savedProjects = await AppCoordinator.shared.savedProjects()
        savedProjectMessage = await AppCoordinator.shared.savedProjectStorageWarning()
    }
}
