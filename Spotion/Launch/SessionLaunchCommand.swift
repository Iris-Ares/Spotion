...
enum SessionLaunchCommand {
    ...
    static func resume(
        ...
        case .codex:
            if let cwd {
                ...
                return "cd \(q(cwd)) && exec \(q(binary)) --cd \(q(cwd)) resume \(q(record.sessionID))"
            }
            // Original directory is gone: defer to Codex's saved-dir logic.
            return "exec \(q(binary)) resume \(q(record.sessionID))"
        case .claude:
            ...
            return "cd \(q(cwd)) && exec \(q(binary)) --resume \(q(record.sessionID))"
        }
    }
    ...
    static func fork(
        ...
        case .codex:
            ...
            let fork = "exec \(q(binary)) fork -- \(q(record.sessionID))"
            guard let cwd else { return fork }
            return "cd \(q(cwd)) && exec \(q(binary)) --cd \(q(cwd)) fork -- \(q(record.sessionID))"
        case .claude:
            guard let cwd else { throw MissingDirectoryError(path: record.cwd) }
            ...
            return "cd \(q(cwd)) && exec \(q(binary)) --resume=\(q(record.sessionID)) --fork-session"
        }
    }
